        ...
        {COURT_ZONES.map((zone) => {
          ...
          return (
            <g key={zone.id} className="group">
              ...
              {stats.attempts > 0 && (
                <>
                  <text x={zone.cx} y={zone.cy - 6} textAnchor="middle" className="text-[12px] font-bold fill-white drop-shadow-md pointer-events-none">{stats.pct}%</text>
                  <text x={zone.cx} y={zone.cy + 8} textAnchor="middle" className="text-[8px] font-medium fill-white drop-shadow-md pointer-events-none opacity-90">({stats.makes}/{stats.attempts})</text>
                </>
              )}
            </g>
          );
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-md text-center">
        ...
        <button onClick={handleGoogleLogin} className="w-full bg-white border border-gray-300 text-gray-700 font-bold py-3 rounded-lg hover:bg-gray-50 transition-colors shadow-sm flex items-center justify-center gap-3">
        ...
        {COURT_ZONES.map((zone) => {
          ...
          return (
            <g key={zone.id} onClick={(e) => { e.stopPropagation(); onZoneClick(zone); }} className="group cursor-pointer">
              <path d={zone.path} className={`stroke-orange-900/10 stroke-1 transition-all duration-150 hover:fill-orange-500/40 active:fill-orange-600/60 ${hasRecord ? 'fill-red-500/20' : 'fill-transparent'}`} />
              <text x={zone.cx} y={zone.cy - (hasRecord ? 8 : 0)} textAnchor="middle" dominantBaseline="middle" className="text-[10px] sm:text-xs fill-gray-700 font-sans font-bold pointer-events-none drop-shadow-md bg-white/50">
                {labelLines.map((line, i) => (<tspan key={i} x={zone.cx} dy={i === 0 ? (labelLines.length > 1 ? "-0.4em" : "0") : "1.2em"}>{line}</tspan>))}
              </text>
              {hasRecord && record && (
                <text x={zone.cx} y={zone.cy + 12} textAnchor="middle" dominantBaseline="middle" className="text-[12px] sm:text-sm fill-gray-400 font-mono font-bold pointer-events-none drop-shadow-md">
...
const InputModal = ({ zone, initialMakes = 0, initialAttempts = 0, onClose, onSave }: { zone: ZoneDef, initialMakes?: number, initialAttempts?: number, onClose: () => void, onSave: (m: number, a: number) => void }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); onSave(makes, attempts); };
...
export default function App() {
    ...
    if (currentUser) {
      ...
      const fetchData = async () => {
        try {
          ...
          console.log("Backend not reachable or Auth failed. Using Mock Data.", error);
  ...
  const handleSaveRecord = async (makes: number, attempts: number) => {
    ...
    const queryDate = formatDateQuery(selectedDate);
    const newRecord = {
      // userIdはバックエンドでトークンから取得するため空文字でOKだが念のため入れる
